...
export const signIn = catchAsyncError(async (req,res,next) => {
    ...
    if(!isFound.verified) return next(new AppError("please verify your account first",401))
    if(isFound && match){
        let loggedIn = await userModel.findByIdAndUpdate( isFound._id, {isActive:true,$unset:{loggedOutAt: 1}}, {new:true});
        console.log(loggedIn);
        let token = jwt.sign({name:isFound.name, userId:isFound._id, role:isFound.role},process.env.VERIFY_SECRET);
...
export const logOut = catchAsyncError(async (req,res,next) => {
    ...
    let decoded = await jwt.verify(token,process.env.VERIFY_SECRET);
    console.log(decoded);
